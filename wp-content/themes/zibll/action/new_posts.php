 ...
 * @LastEditTime: 2022-12-05 19:13:41
...
function zib_ajax_newpost_save_featured($post_id = 0, $echo_success = true)
{
    ...
    switch ($type) {
        case 'video': //视频
            if ($data['url']) {
                update_post_meta($post_id, 'featured_video', $data['url']);
            }
            if ($data['pic']) {
                update_post_meta($post_id, 'cover_image', $data['pic']);
                update_post_meta($post_id, 'thumbnail_url', $data['pic']);
            }
            break;

        case 'slide': //幻灯片
            if ($data['ids']) {
                update_post_meta($post_id, 'featured_video', false);
                update_post_meta($post_id, 'featured_slide', implode(',', $data['ids']));
            }
            break;
        ...
        case 'image': //图像
            if ($data['url']) {
                update_post_meta($post_id, 'featured_video', false);
                update_post_meta($post_id, 'featured_slide', false);
                update_post_meta($post_id, 'cover_image', $data['url']);
                update_post_meta($post_id, 'thumbnail_url', $data['url']);
            }
            break;
        ...
        case 'close': //删除
            update_post_meta($post_id, 'featured_video', false);
            update_post_meta($post_id, 'featured_slide', false);
            update_post_meta($post_id, 'cover_image', false);
