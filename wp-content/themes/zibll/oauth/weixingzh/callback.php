 ...
 * @LastEditTime: 2022-12-06 21:18:20
 */
...
switch ($action) {
    ...
    case 'callback':
        ...
        if ($callback) {
            ...
            //储存临时数据
            $weixingzh_event_data = get_option('weixingzh_event_data'); //读取临时数据
            //清理过期数据，避免数据过大
            ...
            update_option('weixingzh_event_data', $weixingzh_event_data, false);
        ...
        //自动回复
        if (!empty($wxConfig['new_subscribe_msg']) && $wxOAuth->callback['Event'] == 'subscribe') {
