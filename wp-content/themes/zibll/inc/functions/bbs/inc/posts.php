 ...
 * @LastEditTime: 2023-01-30 22:32:00
...
function zib_bbs_get_posts_lists_cover($post)
{
    ...
    if (_pz('bbs_posts_cover_opt', true, 'lists_video_s')) {
        ...
        if ($video) {
            ...
            $pic_url  = get_post_meta($post->ID, 'cover_image', true);
    ...
    $cover = $cover ?: zib_bbs_get_posts_slide_cover($post->ID);
    $cover = $cover ?: zib_bbs_get_posts_image_cover($post->ID);
...
{
    return zib_bbs_get_posts_not_allow_view($post) ? false : true;
}
...
function zib_bbs_get_posts_not_allow_view($post = null)
{
    ...
    $static_data[$post->ID] = $not_allow_view;
